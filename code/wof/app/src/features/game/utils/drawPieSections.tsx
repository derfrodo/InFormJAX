...
import { WHEELPARTS } from "../../config/constants/WHEELPARTS";
import { anglePart } from "../../config/constants/anglePart";
import { anglePartHalf } from "../../config/constants/anglePartHalf";

export async function drawPieSections<T extends WheelValue>(
  ...
  for (let i = 0; i < WHEELPARTS; i++) {
    ...
    if (data.imageText) {
      const fontSize = (radius / 6) * devicePixelRatio;
      ...
      const distance =
        dvRad - fontSize  - 40 * devicePixelRatio;
